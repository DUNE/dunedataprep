...
int test_AdcChannelDumper(bool useExistingFcl =false) {
  const string myname = "test_AdcChannelDumper: ";
#ifdef NDEBUG
  ...
  string fclfile = "test_AdcChannelDumper.fcl";
  if ( ! useExistingFcl ) {
    cout << myname << "Creating top-level FCL." << endl;
    ofstream fout(fclfile.c_str());
    fout << "tools: {" << endl;
    fout << "  mytool: {" << endl;
    fout << "    tool_type: AdcChannelDumper" << endl;
    fout << "    FileName: \"\"" << endl;
    fout << "    NewFile: false" << endl;
    fout << "    Prefix: \"ADC dump: \"" << endl;
...
int main(int argc, char* argv[]) {
  ...
  return test_AdcChannelDumper(useExistingFcl);
}
