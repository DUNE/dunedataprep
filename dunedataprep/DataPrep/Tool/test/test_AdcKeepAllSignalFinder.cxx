...
int test_AdcKeepAllSignalFinder(bool useExistingFcl =false) {
  const string myname = "test_AdcKeepAllSignalFinder: ";
#ifdef NDEBUG
  ...
  string fclfile = "test_AdcKeepAllSignalFinder.fcl";
  if ( ! useExistingFcl ) {
  ...
  assert( data.rois[0].first == 0 );
  assert( data.rois[0].second == 99 );
...
int main(int argc, char* argv[]) {
  ...
  return test_AdcKeepAllSignalFinder(useExistingFcl);
}
